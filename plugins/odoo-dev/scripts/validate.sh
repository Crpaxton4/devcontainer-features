...
# --- 20. every .lsp.json command is a name the Feature generates -------------------
# .lsp.json names the language server's executable by bare name, and Claude Code
# looks that name up on PATH. The name is produced in a different tree: a heredoc in
# the personal-features Feature's install.sh writes /usr/local/bin/odoo-ls-server and
# chmods it. The two files are a plugin file and a Feature file, so they can be
# changed in separate PRs by people with no reason to open the other.
#
# Claude Code's failure mode for a command that is not on PATH is a language server
# that never starts — no diagnostics, no go-to-definition, and no error naming the
# cause. Nothing else in this repo reads .lsp.json at all: `claude plugin validate`
# reads the manifest only (measured against 2.1.252) and never opens it. So this gate
# is the only thing holding the name together across the two trees.
#
# WHAT IT PROVES: install.sh writes a file of that name into a bin directory.
# WHAT IT DOES NOT PROVE: that the write runs, that the generated script works, or
# that the Feature is installed in the container the session opens in. It is a name
# check, and it is deliberately the cheap one — it catches the rename, which is the
# failure that is otherwise completely silent.
gate ".lsp.json commands are generated by the Feature"
lsp_json="$ROOT/.lsp.json"
lsp_repo_root="$(cd "$ROOT/../.." && pwd)"
lsp_install="$lsp_repo_root/devcontainer-features/src/personal-features/install.sh"
if [ ! -f "$lsp_json" ]; then
  bad "${lsp_json#"$lsp_repo_root"/} is missing — the plugin registers no language server without it"
elif [ ! -f "$lsp_install" ]; then
  bad "the Feature that generates the LSP command is gone: ${lsp_install#"$lsp_repo_root"/}"
  detail "if install.sh moved, repoint this gate at its new path. A command nothing"
  detail "generates is a language server that never starts, silently."
else
  # Every "command" value at any depth, one per line. An argv-array command counts as
  # its first element, which is the executable. Malformed JSON prints nothing and is
  # reported as "declares no command" below rather than aborting the gate.
  lsp_cmds="$(node -e '
    const push = (out, v) => {
      if (typeof v === "string" && v.trim()) out.push(v.trim());
      else if (Array.isArray(v) && typeof v[0] === "string" && v[0].trim()) out.push(v[0].trim());
    };
    const walk = (n, out) => {
      if (Array.isArray(n)) n.forEach(v => walk(v, out));
      else if (n && typeof n === "object") {
        for (const [k, v] of Object.entries(n)) {
          if (k === "command") push(out, v);
          else walk(v, out);
        }
      }
      return out;
    ...
    try {
      const o = JSON.parse(require("fs").readFileSync(process.argv[1], "utf8"));
      process.stdout.write(walk(o, []).join("\n"));
    } catch (e) { /* no output: reported as "declares no command" */ }
  ' "$lsp_json" 2>/dev/null)"
  if [ -z "$lsp_cmds" ]; then
    bad ".lsp.json declares no command — nothing tells Claude Code what to launch"
    detail "either every command is missing, or the file is not valid JSON and node"
    detail "could not read one out of it."
  else
    lsp_n=0
    while IFS= read -r lsp_cmd; do
      [ -n "$lsp_cmd" ] || continue
      lsp_n=$((lsp_n + 1))
      lsp_base="${lsp_cmd##*/}"
      case "$lsp_base" in
        # Anything outside this set would also be a grep metacharacter below, so an
        # unexpected name is reported rather than quietly matched as a pattern.
        ""|*[!A-Za-z0-9._-]*)
          bad "command '$lsp_cmd' is not a plain executable name — this gate cannot check it"
          detail "keep it a bare name that the Feature puts on PATH, or extend this gate."
          continue ;;
      esac
      lsp_where="$(grep -nE "/bin/${lsp_base}([^A-Za-z0-9_.-]|\$)" "$lsp_install" \
                   | cut -d: -f1 | paste -sd, -)"
      if [ -n "$lsp_where" ]; then
        ok "$lsp_cmd is written into a bin directory by install.sh (line(s) $lsp_where)"
      elif grep -qF -- "$lsp_base" "$lsp_install"; then
        ok "$lsp_cmd appears in install.sh"
        detail "NOTE: all this proves is that the literal name occurs somewhere in that"
        detail "file — no line writes it into a bin directory, so a mention in a comment"
        detail "would satisfy this. Treat it as weaker than the check above."
      else
        bad "$lsp_cmd is named in .lsp.json but appears nowhere in install.sh"
        detail "Claude Code resolves this on PATH; a name nothing generates is a server"
        detail "that never starts and never says why. Rename both files in one change."
        detail "install.sh's own comment above the heredoc records the coupling."
      fi
    done <<< "$lsp_cmds"
