"""Core-owned task-tracker vocabulary: FSM states, runs, events, and windows.

Promoted from ``odoo_sdk.state.models`` by #718 (ADR-005 amendment): these
frozen-shaped dataclasses, the :class:`TaskState` FSM enum, its error
taxonomy, and the repo display vocabulary are *domain* types — every layer
speaks them (the TUI renders :class:`EventRecord` rows, MCP tools branch on
:class:`TaskState`, billing bills :class:`SessionWindow` spans), so their
home is core, not the SQLite persistence package that happens to store them.
The module stays pure stdlib so importing the vocabulary never drags in
persistence.

``odoo_sdk.state.models`` remains importable as a ``sys.modules``-aliasing
compat module (not deprecated — it is the public re-export path
``odoo_sdk.state`` builds on), so every historical import and attribute
patch keeps reaching these exact objects.
"""
...
# (:meth:`~odoo_sdk.state.LocalStateClient.derive_sessions_overlapping`), and
# unlike the old ``"\x00agent"`` sentinel it carries no control character into
# JSON, MCP responses, or a rendered TUI screen. It is also exactly what the
# in-Python derivation (:mod:`odoo_sdk.sessionization.transform`) already
# produced, so the two paths no longer diverge for the same input.
AGENTLESS_REPO = ""
...
def format_repo_label(repo: Optional[str]) -> str:
    ...
    they agree, instead of each masking the absent value independently. Promoted
    to the core vocabulary with the rest of this module (#718) so the TUI names
    it without importing the persistence layer; ``odoo_sdk.state.db`` re-exports
    it unchanged.
    """
