...
or fastmcp 4's two-phase sampling flow (return an input-required result, read
``ctx.input_responses`` on the continuation — #664) before delegating
(``start_task`` does; ``stop_task`` is fully automatic since #623 — its
description elicitation was removed as a no-op gate).
