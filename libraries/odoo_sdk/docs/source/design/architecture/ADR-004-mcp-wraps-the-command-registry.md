...
- **Prompts are registered alongside tools, and only the dynamic ones.** `register_builtin_prompts` (`odoo_sdk/mcp/prompts/builtin`) registers the builtin prompt set to the same FastMCP instance. Since #784 that set is exactly the two prompts whose text is computed at call time from SDK state (`implement_task`, which composes `get_task`, and `report_incident`, which reads the resolved environment). Static instructional prose is *not* an SDK concern: the five consulting skills are owned solely by the `odoo-dev` plugin (`plugins/odoo-dev/skills/`), which is the single source for them. The SDK ships no skill data and serves no skill resources.
...
- Two composition tools (`start_task`, `stop_task`) ship. Two prompts ship: `implement_task` and `report_incident`. The five static consulting prompts (`discovery_notes`, `fibonacci_estimate`, `odoo_code_review`, `odoo_design_doc`, `odoo_quote`), the packaged `odoo_sdk/skills/` data they read, the `SkillsDirectoryProvider` wiring and the `odoo-sdk sync-skills` CLI were all removed in #784 (2026-09-25): the plugin's `plugins/odoo-dev/skills/` tree is now the single source for that prose, so there is no second copy to keep in parity. (`client_status_report` had already been purged in #712.)
