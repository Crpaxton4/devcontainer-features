...
stateDiagram-v2
    ...
    RUNNING --> STOPPED : stop_task (bills)
    AWAITING_ANSWERS --> STOPPED : stop_task (bills)
...
leaves `aborted_at` unset and bills the elapsed time.
...
| `RUNNING`          | `stop_task`               | `STOPPED`          | Session exists in RUNNING state; bills     |
| `AWAITING_ANSWERS` | `stop_task`               | `STOPPED`          | Answers indicate no changes needed; bills  |
