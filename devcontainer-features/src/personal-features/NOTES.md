...
- `~/.config/odoo-dev` (host) → `/usr/local/share/odoo-dev` (container) — `ODOO_DEV_STATE_DIR` points here, so the odoo-dev plugin's state survives rebuilds: this is where its per-task handoff artifacts (`tasks/<task_id>/`), `repo-map.json`, `releases/` and `upgrade-lessons/` now live. The plugin resolves `${ODOO_DEV_STATE_DIR:-$HOME/.local/share/odoo-dev}`, and in a container `$HOME` is image storage, not a mount — so before this row an in-progress multi-session task lost its entire handoff chain on the next rebuild, silently: the next session resolved the same path, found it empty, and started over (#884). Mode `0700`: it holds no credentials, but it does hold client task artifacts. The mount is also the whole answer to #885 — plugin state is no longer inside any repository, so there is nothing for a `.gitignore` or a machine-wide excludes entry to ignore.
