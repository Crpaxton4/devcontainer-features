...
**One hub for the machine, not one per container (#898).** mempalace grants the *MCP writer lease* to exactly one process per palace, and it arbitrates that with a `flock` on a file under the palace root. The palace root is a **host bind mount that every devcontainer on this machine sees**, so the lock is host-global — arbitrated by the host kernel across containers, not within one. The previous design put a hub in each container, bound to `127.0.0.1:8765` and probed on that same loopback, which is a scope mismatch it could not survive: the second container to start was refused the flock, burned its five-restart budget in about 45 seconds on a condition no retry can change, gave up, and every session in it then served its own palace copy with only the first to mutate able to write —

```
MCP error -32001: Peer MCP writer active; this server is read-only for mutating tools
```

Two smaller defects came with it. The lock is never reclaimed: mempalace's own reaper skips `mine_palace_*.lock` by design (#897), so a holder that dies without its `finally` leaves a file behind — and the refusal message quotes the *recorded* holder PID from that file, which is meaningless in another PID namespace and is usually not even current, so it tells an operator to kill a process they cannot find. And the start probe's 60s timeout always expired *after* the supervisor had already given up, overwriting the accurate `gave_up` with a vaguer `no_response` (#921).

**So there is now one long-lived hub container for the host**, started over the host Docker socket from `postStartCommand` and shared by every devcontainer. That is why the Feature `dependsOn` `docker-outside-of-docker`, and why this repo's own `.devcontainer` moved off `docker-in-docker`: a *nested* daemon would give each devcontainer its own private "shared" hub and reproduce the bug one level up.

**`docker-outside-of-docker` is passed `moby: false`, in `dependsOn` and in this repo's own `.devcontainer`.** The Feature defaults to `moby: true`, which installs Debian's `moby-cli`/`moby-engine` packages — and those do not exist on Debian **trixie**. The Feature does not fall back; it hard-fails the build with *"The 'moby' option is not supported on debian 'trixie'"*, which took out the `node:lts-trixie` and `javascript-node:latest` images on the first attempt at this change. `false` installs Docker CE's `docker-ce-cli` instead, which is the same client against the same socket. The two declarations must carry the *same* option value, or the two references resolve to different installs of one Feature.

### Discovery: no plugin change, no MCP registration

The existing stdio proxy in each container finds the hub by itself, unmodified. The stdio registration is **not ours to change** — it lives in the upstream plugin's own `.mcp.json` under `$CLAUDE_CONFIG_DIR/plugins/cache/mempalace/…`, which this repo does not ship and which `sync-claude-mcp`'s `claude plugin update mempalace@mempalace` overwrites on every container create. Four facts, each read out of the installed mempalace 3.9.0 in site-packages rather than out of the docs, are what make that work:

- **`serverinfo.json` stores the `--host` value verbatim.** `server_registry.write_serverinfo` records the bind host as given, and `client_base_url` rewrites it to loopback *only* for the wildcard binds (`_WILDCARD_HOSTS = {"0.0.0.0", "::", "[::]"}`). Binding the hub `--host mempalace-hub` therefore publishes an address every sibling container can dial — which is exactly what binding `0.0.0.0` could not do, and why the hub gets a `--hostname`/`--network-alias` name rather than a wide bind.
- **The server is PID 1 in its own container.** `read_live_serverinfo` discards a record whose recorded PID is not alive, and `_pid_alive` is `os.kill(pid, 0)` — namespace-local, which is what made a sibling's hub look dead. `sh -c` execs into `python`, and `cli.cmd_serve` then `os.execve`s the real server, so the PID never changes: the hub is PID 1, and PID 1 exists in every namespace. The check is now true for the right reason instead of by luck, which is also why the container runs **without `--init`**.
- **The state directory key matches.** `server_state_dir` is `$HOME/.mempalace/server/<sha256 of the canonical palace path>[:24]`. The hub is given the palace at *the same absolute path* the containers use (`/usr/local/share/mempalace/palace`) and a `$HOME` of `/usr/local/share/mempalace/hub/home`, whose `.mempalace` is a symlink back onto the mount. Token, `serverinfo.json` and the palace lock then all land in the one directory every container already reads.
- **The bearer token distributes itself.** `cmd_serve` auto-generates a strong token `0600` into that directory for any non-loopback bind, and `load_server_tokens` reads it from the same place. Nothing has to carry a secret between containers; the mount already does.

### `mempalace-hub-up`

`postStartCommand` runs `mempalace-hub-up`. It is idempotent, safe on every container start, and **never fatal**: every path records a state into the `mempalace_hub` key of the #806 provision marker and exits 0. A container with no hub is degraded — each session serving its own palace copy, one writer among them — not broken, and never worth failing container start over.

| Subcommand | What it does |
| --- | --- |
| `up` (default) | Reconcile the hub: socket, identity, network, dependency-set label, lock, run, wait for `/healthz`. |
| `status` | Re-probe `/healthz` and print the state the marker last recorded. |
| `down` | `docker stop` the hub. |
| `logs` | `docker logs` the hub; extra arguments pass straight through (`mempalace-hub-up logs -f`). |
...
| `no_docker` | No `docker` binary — `docker-outside-of-docker` is missing. |
| `no_socket` | No `/var/run/docker.sock`, or `docker info` does not answer. |
| `self_not_found` | This container's id, or the host path behind the palace mount, could not be resolved. |
| `network_failed` | The `mempalace` network could not be created or joined. |
| `pull_failed` | The hub image could not be pulled, or `docker run` failed. |
| `lock_held` | Something live holds the palace lock; a hub would be refused the lease. |
...
The record keeps the key name and the **merge** semantics the old supervisor's `hub_record` used: a previous writer's fields under `mempalace_hub` are carried forward rather than erased, and `sync-claude-mcp` carries the whole key through every provision (#868). #921 is gone with the two racing timers that caused it — there is exactly one writer of this key now, and it writes one terminal state per run.
...
**The lock is probed, never removed.** Before creating a hub, `mempalace-hub-up` takes a non-blocking `flock` on each `mine_palace_*.lock` under the mount and releases it immediately. A file surviving on the mount proves nothing — that is the whole of #897 — but a refused `flock` means a *living* process holds it, and starting a hub into that would crash-loop against a condition no retry can change. So it records `lock_held`, names the file, and stops. **Never delete a lock file by hand.**

### Recovering

`mempalace-hub-up status` first. Then:

- `live` — nothing to do. `mempalace_status` from any two containers should agree.
- `starting` — it is installing its dependency set. `mempalace-hub-up logs -f`, then `status` again.
- `lock_held` — find the holder and stop it; `mempalace-hub-up down` if it is an old hub. Do not remove the lock.
- anything else — the state names the cause. Fix it and re-run `mempalace-hub-up`.
- Stuck, or a Feature release moved the pins: `mempalace-hub-up down` then `mempalace-hub-up`. The label comparison recreates the container when the dependency set moved; `down` then `up` is the only recovery step that should ever be needed, and neither touches the palace.

**No new persisted path.** The hub's registry record, its bearer token, its `$HOME`, the staged requirements file and the palace itself all live under the existing `~/.mempalace` → `/usr/local/share/mempalace` mount; the liveness record lives in the `~/.claude` → `/usr/local/share/claude-home` mount the provision marker already uses. So `persisted-paths.tsv`, `devcontainer-feature.json`'s `mounts`/`containerEnv`, `setup.sh` and `setup.ps1` are untouched by this.

**Security.** Mounting the host Docker socket grants a devcontainer root-equivalent control of the host daemon. That is accepted here deliberately: it is the only route a Feature has to a sibling container, and the alternative — electing a hub among the devcontainers themselves — buys failover and election-flapping in exchange. The hub binds a name inside a user-defined Docker network, not a host interface, and keeps the auto-generated bearer token; `--allow-insecure` is never passed.
